...
def get_args_parser():
    ...
    parser.add_argument('--adam_betas', default=0.9, type=float)
    ...
    parser.add_argument('--giou', default=3, type=float)
    parser.add_argument('--BCEWithLogitLoss_up', default=0.2, type=float)
    parser.add_argument('--BCEWithLogitLoss_down', default=0.2, type=float)
    parser.add_argument('--binary_tversky', default=0.2, type=float)
    parser.add_argument('--BCEWithLogitLoss', default=1, type=float)
    parser.add_argument('--TverskyLoss', default=1, type=float)
    parser.add_argument('--DiceLoss', default=1, type=float)
    parser.add_argument('--FocalLoss', default=3, type=float)
    parser.add_argument('--TverskyLoss_a', default=1, type=float)
    parser.add_argument('--TverskyLoss_b', default=1, type=float)

    parser.add_argument('--other_loss_coefs', default={}, type=float)

    # dataset parameters
    parser.add_argument('--blip_checkpoint_path')
    ...
    parser.add_argument('--use_checkpoint_optimizer')
...
def main(args):
 ...
 # batch_sampler_train = torch.utils.data.BatchSampler(
    ...
    if args.model_name == 'vltvg':
    ...
    elif args.model_name == 'CAVG_V3' or 'Stage1':
        # backbone_param = [p for p in model_without_ddp.BLIP.visual_encoder.parameters() if p.requires_grad]
        # backbone_param_n = [n for n, p in model_without_ddp.BLIP.visual_encoder.named_parameters() if p.requires_grad]
    
        # bert_param = [p for p in  model_without_ddp.BLIP.bert.parameters() if p.requires_grad]
        # bert_param_n = [n for n, p in model_without_ddp.BLIP.bert.named_parameters() if p.requires_grad]

        # rest_param = [p for n, p in model_without_ddp.BLIP.bert.named_parameters() if p.requires_grad and (n not in backbone_param_n) and (n not in bert_param_n)]

        # backbone_prefix = "BLIP.visual_encoder."
        # bert_prefix = "BLIP.bert."

        # # 使用完整路径名称更新backbone_param_n和bert_param_n列表
        # backbone_param = [p for p in model_without_ddp.BLIP.visual_encoder.parameters() if p.requires_grad]
        # backbone_param_n = [backbone_prefix + n for n, p in model_without_ddp.BLIP.visual_encoder.named_parameters() if p.requires_grad]
        
        # bert_param = [p for p in  model_without_ddp.BLIP.bert.parameters() if p.requires_grad]
        # bert_param_n = [bert_prefix + n for n, p in model_without_ddp.BLIP.bert.named_parameters() if p.requires_grad]

        # # 现在rest_param应该从整个BLIP模型中筛选参数，排除上面两个列表中的参数
        # rest_param = [p for n, p in model_without_ddp.BLIP.named_parameters() if p.requires_grad and (n not in backbone_param_n) and (n not in bert_param_n)]
        backbone_param = set(p for p in model_without_ddp.BLIP.visual_encoder.parameters() if p.requires_grad)
        bert_param = set(p for p in model_without_ddp.BLIP.bert.parameters() if p.requires_grad)

        # 排除这些参数，选择剩余的参数
        rest_param = [p for p in model_without_ddp.parameters() if p.requires_grad and p not in backbone_param and p not in bert_param]
        ...
        param_dicts = [{'params': list(rest_param)}, # base_lr
                    {'params': list(backbone_param), 'lr': args.lr_backbone}, # base_lr/100.
                    {'params': list(bert_param), 'lr': args.lr_bert},] # base_lr/100.
        ...
        total_steps = int((len(dataset_train) // args.batch_size) * args.epochs) if len(dataset_train) % args.batch_size == 0 else (len(dataset_train) // args.batch_size + 1) * args.epochs
        warmup_steps= args.warm_up_ratio * total_steps   
        ...
        lr_scheduler = WarmupLinearSchedule(optimizer, warmup_steps=warmup_steps, t_total=total_steps)
    # else:
        
    #     # embedding_param = [p for p in model_without_ddp.embeddings.parameters() if p.requires_grad]
    #     # rest_param = [p for p in model.parameters() if p.requires_grad and not (p in embedding_param)]

    #     # param_dicts = [{'params': rest_param}, # base_lr
    #     #                 {'params': embedding_param, 'lr': args.lr_bert/10},] 
    ...
    #     # # lr_scheduler = aWrmupLinearSchedule(optimizer, warmup_steps= warm_up_ratio * total_steps, t_total= total_steps)
    #     # # python /home/HaichengLiao/VLTVG_region_proposal/train_talk2car_cross_cos.py --config /home/HaichengLiao/VLTVG_region_proposal/configs/CAVG_V2_cos.py --model_name=='CAVG_V2' --device 'cuda:2' --checkpoint /home/HaichengLiao/VLTVG_region_proposal/work_dirs/CAVG_v2/checkpoint0002.pth --lr 1e-6
    #     # T_max = 5 
    ...
    #     # 每一个epoch中有多少个step可以根据len(DataLoader)计算：total_steps = len(DataLoader) * epoch
    #     warm_up_ratio = 0.15
    #     # python /home/HaichengLiao/VLTVG_region_proposal/train_talk2car_warm.py --config /home/HaichengLiao/VLTVG_region_proposal/configs/CAVG_V2_warm.py --model_name=='CAVG_V2' --device 'cuda:1' --checkpoint /home/HaichengLiao/VLTVG_region_proposal/work_dirs/CAVG_v2/checkpoint0002.pth --lr 1e-6
    ...
    if args.resume:
        ...
        if not args.eval and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
            if args.use_checkpoint_optimizer:
    ...
    if args.start_epoch < args.freeze_epochs and args.freeze_modules:
        ...
        param_freezer.freeze(model_without_ddp.BLIP)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) == args.freeze_epochs and args.freeze_modules:
            ...
            print(f'Unfreeze weights: {args.freeze_modules}')
            param_freezer.unfreeze(model_without_ddp.BLIP)
...
# python /home/ming/Documents/VLTVG/trian_talk2car_corss_cos.py --config /home/ming/Documents/VLTVG/configs/CAVG_V2.py --checkpoint /home/ming/Documents/VLTVG/checkpoints/checkpoint0002.pth --model_name CAVG_V2 --lr 1e-7
...
# python /home/tam/Documents/RSDLayerAttn/VLTVG_RegionProposal/train_talk2car.py --config /home/tam/Documents/RSDLayerAttn/VLTVG_RegionProposal/configs/VLTVG_R101_talk2car_Region_proposal.py
# python /home/tam/Documents/RSDLayerAttn/VLTVG_RegionProposal/train_talk2car.py --config /home/tam/Documents/RSDLayerAttn/VLTVG_RegionProposal/configs/VLTVG_R101_talk2car_Region_proposal.py --model_name 'CAVG_without_score_and_gpt'
# pip install torch==1.8.0 torchvision==0.9.0
# pip3 install torch==1.8.2 torchvision==0.9.2 torchaudio==0.8.2 --extra-index-url https://download.pytorch.org/whl/lts/1.8/cu111
# # python -m torch.distributed.launch --nproc_per_node=1 --use_env train.py --config /home/tam/Documents/RSDLayerAttn/VLTVG/configs/VLTVG_R50_gref.py 
