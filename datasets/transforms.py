...
max_size: 裁剪区域的最大尺寸。
max_cnt: 尝试裁剪的最大次数。
如果 func 是 'area'，使用 check_area 方法，并设置面积阈值 area_thres。
如果 func 是 'iou'，使用 check_iou 方法,并设置交并比IoU阈值 iou_thres。
如果 func 不是这两个值，抛出 NotImplementedError 异常。
'''
...
class MyRandomResize(object):
    def __init__(self, sizes, resize_long_side=True, record_resize_info=False,new_h=640,new_w=640):
        self.sizes = sizes
        self.resize_long_side = resize_long_side
        self.new_h = new_h
        self.new_w = new_w
    ...
    def __call__(self, img, target):
        ...
        img = F.resize(img, (self.new_h, self.new_w))


        ratio_h, ratio_w = float(self.new_h) / h, float(self.new_w) / w
        ...
        if self.record_resize_info:
            target['orig_size'] = torch.as_tensor([h, w], dtype=torch.float32)
            target['ratio'] = torch.as_tensor([ratio_h, ratio_w], dtype=torch.float32)
            target['size'] = torch.as_tensor([self.new_h, self.new_w], dtype=torch.float32)

        return img, target
    
class MyNormalizeAndPad(object):
    def __init__(self, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225], size=640,
                 aug_translate=False, center_place=False, padding=True):
        self.mean = mean
        self.std = std
    ...
    def __call__(self, img, target):
        ...
        out_mask = torch.zeros((900,900), dtype=torch.bool)
        target['mask'] = out_mask
        ...
        # box = box_xyxy_to_cxcywh(box)
        # box = box / torch.tensor([out_w, out_h, out_w, out_h], dtype=torch.float32)
        #target['size'] = torch.as_tensor([img.h, img.w], dtype=torch.float32)
        box = box / torch.tensor([1800, 900, 1800, 900], dtype=torch.float32)
        target['bbox'] =  box_xyxy_to_cxcywh(box)
...
PIL_TRANSFORMS = {
    ...
    'MyRandomResize':MyRandomResize,
    "MyNormalizeAndPad":MyNormalizeAndPad,
}