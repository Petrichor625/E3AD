...
from volta.encoders import BertForVLTasks,BertForVLTasks_UseFeature
...
def parse_args():
    ...
    parser.add_argument("--ues_feature", type=bool,default=True)
...
def main():
    ...
    default_gpu = True if torch.cuda.is_available() else False
    # Load config
    ...
    # Model
    if args.ues_feature:
        model = BertForVLTasks_UseFeature.from_pretrained(args.from_pretrained, config=config, task_cfg=task_cfg, task_ids=[task], probe_layer_idx=args.probe_layer_idx)
    else:
        import sys
        sys.exit(0)
